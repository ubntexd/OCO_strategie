...
async function loadTab(tab) {
  ...
  try {
    if (tab === 'trades') {
      ...
      panel.innerHTML = `<div class="card"><h2>Événements récents</h2><pre>${JSON.stringify(rows, null, 2)}</pre></div>`;
    } else if (tab === 'config') {
      panel.innerHTML = `<div class="card"><h2>Configuration à chaud</h2>
        <p>Envoi via Redis pub/sub — utilisez le MCP <code>set_config</code> ou POST /config sur le bot.</p>
        <ul>
          <li>Paramètres autorisés : MAX_SPREAD, MAX_CONSEC_LOSS, TP_BRUT, SL_BRUT, ATR_TP_MULT</li>
          <li>Reset daily : POST /config {key:"reset_daily",value:"1"} sur bot:4001/4002/4003</li>
        </ul>
      </div>`;
    } else if (tab === 'health') {
      const rows = await api('/health');
      panel.innerHTML = `<div class="card"><h2>Health Dashboard</h2><pre>${JSON.stringify(rows, null, 2)}</pre></div>`;
    } else {
